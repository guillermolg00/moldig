...
- `.codex/config.toml` — 6 top-level keys mirrored from the real file, every value
  ...
  **not** copied: `[mcp_servers.x]` (stdio: `command`, `args`, `cwd`, `env` sub-table, `tools.<tool>`
  ...
  `entry-N`, at most 3 per parent; arrays are capped at 3 items. The `entry-N` counter runs across a
  parent's scalars **and** its sub-tables (`[desktop]` ends at `entry-6`, its sub-table is
  `[desktop.entry-7]`): reusing a number would redefine a scalar as a table, which is not valid
  TOML 1.0 and which no TOML parser — Codex's included — will read.
...
- `.codex/skills/` — three generations side by side: `find-skills` is a **symlink** to
  ...
  `vercel-react-best-practices/` is a real copy (older layout) with `SKILL.md` (121 lines,
  5,340 bytes — 117 of them body), a 60 KB `AGENTS.md` **payload** (2249 lines), `metadata.json`
  (real keys), `rules/rule-a.md` and a Codex `agents/openai.yaml`; `.system/skill-creator/` is a
  bundled system skill (228 lines, 223 of them body) next to the `.codex-system-skills.marker`
  file.
