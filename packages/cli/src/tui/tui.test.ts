...
describe("overview", () => {
  it("renders the headline, the eight categories and the projects line", () => {
    const screen = open({ screen: "overview" });
    const frame = screen.frame();
    expect(frame).toContain("Headline — project-a (encloses cwd)");
    ...
    expect(frame).toContain("Projects: 1 present · 1 gone · 0 unreachable · 1 harness");
    screen.unmount();
  });

  it("enter opens the category, p the projects, s the selection panel, esc pops back", async () => {
    const screen = open({ screen: "overview" });
    await screen.press(ENTER);
    expect(screen.frame()).toContain("moldig · duplicate · 2 findings");
    await screen.press(ESC);
    expect(screen.frame()).toContain("moldig · overview");
  ...
  it("? opens the help overlay with the real key map and any key closes it", async () => {
    const screen = open({ screen: "overview" });
    await screen.press("?");
    expect(screen.frame()).toContain("toggle every visible tickable row");
    expect(screen.frame()).toContain("any key closes help");
    await screen.press("x");
    expect(screen.frame()).toContain("Headline — project-a");
    screen.unmount();
  });
});
...
describe("projects", () => {
  it("sorts present projects by session cost and keeps the gone ones collapsed", async () => {
    ...
    expect(frame).toContain("directory gone; what every harness left about it stays together");
...
describe("items", () => {
  it("groups the sections, marks human-owned rows as advice and shows the cache groups", () => {
    ...
    expect(frame).toContain("advice · o open");
  ...
  it("space on a human-owned row marks it for Open and d reports why a settings file cannot go", async () => {
    const screen = open({ screen: "items", container: harnessId(), title: "user" });
    await screen.press(" ");
    expect(screen.frame()).toContain("→ open");
    await screen.press("h", "d");
...
describe("category findings", () => {
  it("lists the findings with severity, container and impact", () => {
    ...
    expect(frame).toContain("action: clean");
    expect(frame).toContain("Claude Code · user scope");
    screen.unmount();
  });
...
describe("item detail", () => {
  it("shows the path, the loaded-by verdict, the memory cap and the actions", () => {
    ...
    expect(frame).toContain("memory-file · user scope · harness-owned");
    expect(frame).toContain("path ~/.claude");
    expect(frame).toContain("(OSC 8 links off in this terminal — press o)");
    expect(frame).toContain("Loaded by (one verdict per harness)");
    expect(frame).toContain("first 200 lines / 25 KB in every session");
    expect(frame).toContain("space clean → Trash");
    expect(frame).toContain("update (no installer) (not available)");
    screen.unmount();
  });
...
describe("selection panel", () => {
  ...
  it("space unselects a row and enter with nothing selected says what to do", async () => {
    ...
    expect(empty.frame()).toContain("nothing selected yet — space ticks harness-owned rows");
    screen.unmount();
    empty.unmount();
  });
...
describe("leaving", () => {
  it("q unmounts and hands back the shareable summary; nothing failed, so the run is 0", async () => {
    ...
    expect(outcome.summary).toContain("every session pays 100 + project-a adds 265 = 365");
    expect(outcome.summary).toContain("Nothing moved (preview): 3 rows selected");
    expect(outcome.summary).toContain("Clean: 3 rows");
