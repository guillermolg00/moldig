 ...
 *
 * Pure text layouts for the ego-graph screen. Every layout returns lines of styled
 * segments that already fit `view.width` (labels truncated with an ellipsis) and
 * `view.height` (lists windowed around the selected row), so the Ink component only
 * paints them. Widths are counted in code points: every glyph used here is one cell
 * wide; a label with East Asian wide characters would misalign a column (prototype).
 */
...
export type LayoutId = "columns" | "outline" | "radial";
export const LAYOUTS: readonly LayoutId[] = ["columns", "outline", "radial"];
...
// ---------- (3) radial: the focus in the centre, up to 8 hop-1 neighbours around it ----------

const SLOT_ORDER = ["E", "W", "N", "S", "NE", "SW", "NW", "SE"] as const;
type Slot = (typeof SLOT_ORDER)[number];

function radialLines(graph: EgoGraph, view: View): Line[] {
  const w = view.width;
  const colW = Math.floor((w - 2) / 3);
  const c1 = colW + 1; // first column of the centre cell
  const c2 = 2 * colW + 2; // first column of the right cell
  const mid = c1 + Math.floor(colW / 2);
  const ctx = ctxOf(graph);
  const hop1 = graph.neighbours.filter((n) => n.hop === 1);
  const slots = new Map<Slot, EgoNeighbour>();
  hop1.slice(0, RADIAL_SLOTS).forEach((n, i) => {
    const slot = SLOT_ORDER[i];
    if (slot) slots.set(slot, n);
  });
  const has = (slot: Slot): boolean => slots.has(slot);

  const cell = (slot: Slot, how: Align): Segment[] => {
    const n = slots.get(slot);
    if (!n) return [blank(colW)];
    let segs = fit(
      [
        seg(`${arrow(n.direction)}${EDGE_GLYPH[n.kind]} `),
        ...nodeSegments(n.node, ctx),
        ...(n.mode !== null ? [seg(` · ${SHORT_MODE[n.mode]}`)] : []),
      ],
      colW,
    );
    if (n.confidence === "medium" || n.confidence === "low") segs = restyle(segs, { dim: true });
    if (n.node.id === view.selectedId) segs = restyle(segs, { inverse: true });
    return align(segs, colW, how);
  };

  const grid = (marks: [number, string][]): Line => {
    const chars = Array.from({ length: w }, () => " ");
    for (const [col, ch] of marks) if (col >= 0 && col < w) chars[col] = ch;
    return { segments: [seg(chars.join(""), { dim: true })] };
  };
  const marks = (pairs: [Slot, number, string][]): [number, string][] =>
    pairs.filter(([slot]) => has(slot)).map(([, col, ch]) => [col, ch]);

  // centre cell: the focus, with rules towards W / E when those slots are filled
  const focusText = fit(restyle(nodeSegments(graph.focus, ctx), { bold: true }), colW - 2);
  const gap = colW - lineWidth(focusText);
  const leftGap = Math.floor(gap / 2);
  const rightGap = gap - leftGap;
  const centreRow: Segment[] = [
    has("W") && leftGap > 0 ? seg(`${"─".repeat(leftGap - 1)} `, { dim: true }) : blank(leftGap),
    ...focusText,
    has("E") && rightGap > 0 ? seg(` ${"─".repeat(rightGap - 1)}`, { dim: true }) : blank(rightGap),
  ];
  ...
  const lines: Line[] = [
    ...
    {
      segments: [...cell("W", "right"), blank(1), ...centreRow, blank(1), ...cell("E", "left")],
    },
    grid(
      marks([
        ["SW", colW + 2, "╱"],
  ...
  const notes: string[] = [];
  if (hop1.length === 0) notes.push("no edges of the legend kinds");
  if (hop1.length > RADIAL_SLOTS) {
    notes.push(`⋯ ${hop1.length - RADIAL_SLOTS} more not drawn (L: outline)`);
  }
  const deeper = graph.neighbours.length - hop1.length;
  if (graph.hops > 1) notes.push(`hop 2 not drawn here (${deeper} nodes; L: outline)`);
  const selected = graph.neighbours.find((n) => n.node.id === view.selectedId);
  if (selected && ![...slots.values()].includes(selected)) {
    notes.push(`selected: ${selected.node.label} (not drawn)`);
  }
  for (const text of notes) lines.push({ segments: fit([seg(text, { dim: true })], w) });
  return lines.slice(0, view.height);
}
